...
# 5. Summarize TFs across datasets
...
# 6. Filter recurrent TFs (in more than half datasets)
...
# 7. Determine direction consistency — within each niche only

my.rec.mph.microglia <- my.rec.mph.microglia %>%
  rowwise() %>%
  mutate(
    Direction = {
      current_tf <- TF_regulon
      tf_signs <- microglia_TFs %>%                
        filter(source == current_tf) %>%
        pull(sign)
      if (length(unique(tf_signs)) == 1) "consistent" else "inconsistent"
    }
  ) %>%
